...
/*
   improve the default behavior parameters to be more active early game
   also up priority of patents... I think
...
{
    public class waffleFriesAI : Abstract
    {
        public waffleFriesAI(GameClient pGame, bool liveGame, bool debugMode) : base(pGame, liveGame, debugMode)
        {
        }
        public override void AI_doActions(bool is_auction)
        {
            ...
            if (DebugMode)
            {
                //Debug.Log($"Hello world! Chips");
                if (LiveGame)
                {
                    // Only invoke AppGlobals functions if its a live game
                    //AppGlobals.GameHUDHelpers.DisplayChatMessage(getPlayer(), false, "Hello world! Chips");
